...
class OneDishShoppingCart extends Component {
	render() {
		const { order, handleOrder, checkout } = this.props;

		return order.map(dish => (
			<div key={dish.name} className="dishContainer">
				<div className="dishContainer__dishName__container">
					...
					{dish.additionals && (
						<span className="dishContainer__dishName--adds">+ extras</span>
					)}
				</div>
				<p className="dishContainer__price">
					<span className="dishContainer__price__quantity">
						{dish.quantity}x
					</span>{' '}
