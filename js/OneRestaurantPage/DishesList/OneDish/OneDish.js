			...
			<>
				{menu.map(dish => (
					<li className="dishesList__item" key={dish.id}>
						<div
							className="oneDishView"
							onClick={() => this.handleOpenDetails(dish.id)}
						>
							...
							<div className="oneDishView__container">
								<p className="oneDishView__container__price">
									{dish.price.toFixed(2)} PLN
								</p>
								{dish.additionals.length ? (
									<p className="oneDishView__container__additionals">
										Additionals <i className="fas fa-plus-circle"></i>
									</p>
								) : (
